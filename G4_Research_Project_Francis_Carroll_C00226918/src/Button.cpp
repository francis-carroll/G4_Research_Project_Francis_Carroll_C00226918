...
Button::Button(Vector2f t_position, string t_text) :
	...
	m_padding(Vector2f(10.0f, 10.0f)),
...
void Button::render(shared_ptr<RenderWindow> t_window)
{
	t_window->draw(m_button);
	t_window->draw(m_buttonText);
}

void Button::handleMouseInput(Event& t_event, shared_ptr<RenderWindow> t_window)
{
...
void Button::setup()
{
	...
	m_buttonText.setPosition(m_position + Vector2f(0.0f, 10.0f));
...
bool Button::collision(shared_ptr<RenderWindow> t_window)
{
...
bool Button::hover(Event& t_event, shared_ptr<RenderWindow> t_window)
{
...
bool Button::click(shared_ptr<RenderWindow> t_window)
{
